...
public class Problem_1987_Opti_Review_Fail {
    ...
    private static Map<Integer, int[][]> cache;
    ...
    private static int findLongestDistance(final Point cur, final int used) {
        final int[][] currentDistances = cache.computeIfAbsent(used, key -> new int[R][C]);

        if (currentDistances[cur.row][cur.col] != 0) {
            return currentDistances[cur.row][cur.col];
        }
        ...
        return currentDistances[cur.row][cur.col] = maxOrDefault(max);
    }
